...
public class ManejadorTablero : MonoBehaviour
{
    ...
    void caidaLogica ()
    {
        ...
        {
            // Inicializamos emptyRow a la última fila
            int emptyRow = maximoFilas - 1;

            // Recorremos cada fila de abajo hacia arriba
            for (int row = maximoFilas - 1; row >= 0; row--)
            {
                // Si la celda actual no está vacía (es decir, está ocupada)
                if (matrizLogica[row, col] != _ficha.VACIO)
                {
                    // Movemos la celda ocupada a la posición de emptyRow si no está ya allí
                    if (emptyRow != row)
                    {
                        // Colocamos la celda ocupada en la posición de emptyRow
